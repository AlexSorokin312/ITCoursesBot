...
public class ProgressSummaryMiddleware : IUpdateMiddleware
{
    ...
    private readonly ILogger<ProgressSummaryMiddleware> _logger;
    ...
    public ProgressSummaryMiddleware(
        ...
        ILogger<ProgressSummaryMiddleware> logger)
    {
        _messageService = msg;
    ...
    public async Task InvokeAsync(UpdateContext ctx, Func<Task> next)
    {
        ...
        try
        {
            _logger.LogInformation("Пользователь {ChatId} запросил сводку прогресса", chatId);
            ...
            {
                _logger.LogError(ex, "Ошибка при сборе текстовой сводки для {ChatId}", chatId);
            ...
            {
                _logger.LogError(ex, "Ошибка при формировании prompt для AI для {ChatId}", chatId);
            ...
            {
                _logger.LogError(ex, "Ошибка запроса к OpenAI для {ChatId}", chatId);
            }
            ...
            {
                _logger.LogWarning("AI не вернул ответ для {ChatId}, отправляем меню", chatId);
                await SafeSendAsync(chatId, "🏠 Главное меню:\nВыберите опцию:", _keyboardBuilder.Build(), ctx.CancellationToken);
            }
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Неожиданная ошибка в ProgressSummaryMiddleware для {ChatId}", chatId);
        }
    }
    ...
    private async Task SafeSendAsync(long chatId, string text, CancellationToken ct)
    {
        ...
        {
            _logger.LogError(ex, "Не удалось отправить сообщение чату {ChatId}: {Text}", chatId, text);
        }
    }
    ...
    private async Task SafeSendAsync(long chatId, string text, InlineKeyboardMarkup markup, CancellationToken ct)
    {
        ...
        {
            _logger.LogError(ex, "Не удалось отправить клавиатуру чату {ChatId}: {Text}", chatId, text);
        }
    }
